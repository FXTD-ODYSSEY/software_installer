...
class MainApplication(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        # NOTE 复选框
        self.QTTabBar = tk.Checkbutton(gen_frame, text='QTTabBar', onvalue=0, command=lambda:sys.stdout.write("checkbox\n"))
        self.QTTabBar.pack(side="top", fill="both", expand=1, padx=5,pady=5)

        self.path_widget = SelectPathWidget(gen_frame,label_text="安装目录", button_text="选择路径",path_text=os.path.dirname(__file__))
        self.path_widget.pack(side="top", fill="both", expand=1, padx=5,pady=5)

        gen_btn = tk.Button(gen_frame,text ="自动安装",command=self.generateDriver)
